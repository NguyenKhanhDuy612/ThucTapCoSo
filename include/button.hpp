...
struct Button
{
	...
	void hienInit(int );// hien thi
	void nutBT();// nut binh thuong
	void nutDanhDau();// thay doi mau cho nó
	void nutChon();// luu nut chon va doi mau
	void showButtonHint();
	void showName() const;
...
void Button::hienInit(int borderColor)
{
...
inline void Button::nutBT()
{
	color = BUTTON_COLOR;
	hienInit(BUTTON_BORDER);
}
inline void Button::nutDanhDau()
{
	color = BUTTON_COLOR_HIGHTLIGHT;
	hienInit(BUTTON_BORDER_HIGHTLIGHT);
}
inline void Button::nutChon()
{
	color = BUTTON_COLOR_CHOSE;
	hienInit(BUTTON_BORDER_CHOSE);
}
...
void iconNut(Button const *menu, int i)
{
	setcolor(BLACK);
	switch (i)
	{
